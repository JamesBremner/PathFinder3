...
namespace raven
{
    namespace graph
    {
        ...
        typedef std::multimap<int, cLink> nodeOutEdgesMap_t;
        ...
        public:
            ...
            void addLink(int u, int v, double cost = 1)
            {
                ...
                it->second.myLink.insert(std::make_pair(v, cLink(cost)));
                if (!myfDirected) {
                    it = myG.find(v);
                    if( it == myG.end() )
                    throw std::runtime_error(
            ...
            /// @param u 
            /// @param v 
            /// @return edge, one of edges between u and v
